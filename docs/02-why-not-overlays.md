...
| ------------------------- | -------------------------------------------------------- | --------------------------------------------------------------------------------- |
| **Portable checks**       | Finds source-supported issues and lists manual review.   | `@accessibility-devkit/core`, `@accessibility-devkit/cli`, and the Python package |
| **Testing & Auditing**    | Catches rendered violations during development and CI.   | `@accessibility-devkit/audit` (axe-core, eslint-plugin-jsx-a11y)                  |
| **Components & Patterns** | Provides structurally sound interaction building blocks. | `@accessibility-devkit/components` (focus trap, skip link, live region)           |
| **Focused utilities**     | Applies explicit preferences and interaction helpers.    | `accommodations`, `motor`, `cognitive`, `language`, `media`, and `motion`         |
